...
export function TwoFactorSetup({
  ...
  const handleContinue = React.useCallback(async () => {
    ...
    setLoading(true);
    try { await onVerify?.(token); } finally { setLoading(false); }
  }, [otp, onVerify]);

  const isComplete = otp.every((d) => d !== "");

  return (
    <div className={cn("flex rounded-2xl overflow-hidden shadow-2xl w-full max-w-[660px] bg-[#1a1a1a]", className)}>
      <div className="relative flex flex-col items-center justify-center gap-5 w-[45%] px-8 py-10 bg-[#141414]">
        ...
        <div className="text-center">
          <p className="text-white font-semibold text-base tracking-wide">Secure Protocol</p>
      ...
      <div className="relative flex-1 flex flex-col justify-center px-8 py-10">
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-2 bg-[#0d0d0d] border border-teal-500/40 rounded-lg px-4 py-3">
            <span className="flex-1 font-mono text-teal-300 text-sm tracking-widest">{backupCode}</span>
            <button onClick={handleCopy} className="text-teal-400 hover:text-teal-300 transition-colors flex-shrink-0">
              <AnimatePresence mode="wait" initial={false}>
                {copied ? (
                  <motion.svg key="check" width="16" height="16" viewBox="0 0 16 16" fill="none"
                    initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }}
                    transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                    <path d="M3 8l3.5 3.5L13 4" stroke="#2dd4bf" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </motion.svg>
                ) : (
                  <motion.svg key="copy" width="16" height="16" viewBox="0 0 16 16" fill="none"
                    initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }}
                    transition={{ type: "spring", stiffness: 400, damping: 25 }}>
          ...
          <p className="text-gray-500 text-xs mt-2">Use this code if you&apos;re unable to scan the QR graphic.</p>
        </div>
            ...
            {otp.map((digit, i) => (
              <input
                ...
                className={cn(
                  ...
                  "transition-all duration-150 focus:border-teal-500/60 focus:bg-[#1e1e1e]",
